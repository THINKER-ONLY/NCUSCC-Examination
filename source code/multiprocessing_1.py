...
def split_matrix(A, B, num_splits):
    ...
    return row_size, row_splits

def parallel_matrix_multiply(A, B, num_splits):
    row_size, row_splits = split_matrix(A, B, num_splits)

    # 创建进程池
    with Pool(processes=num_splits) as pool:
        # 创建任务列表
        tasks = [(row, B) for row in row_splits]

        # 执行并行计算
        results = pool.starmap(matrix_multiply, tasks)

    # 合并结果
    final_result = np.zeros((A.shape[0], B.shape[1]), dtype=np.float32)
    for i in range(num_splits):
        start_row = i * row_size
        end_row = (i + 1) * row_size
        final_result[start_row:end_row, :] = results[i]
